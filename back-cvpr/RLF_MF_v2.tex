...
%As for outlier detection, our framework is shown in Figure~\ref{fig:framework} (notations are introduced shortly).
...
%as shown in Figure~\ref{fig:framework} (notations are introduced shortly),
...
Furthermore, most existing matrix factorization optimization methods,
however, only consider smooth loss functions, rather than the nonsmooth loss.
...
%% Feature 
%Some algorithms exploit the early fusion methods, \textit{i.e.} feature fusion,
%to improve the performance of their CNN architectures.
...
Then by applying each single classifier $f_j$ on the entire testing data, one can derive an indicator matrix $\bL_j \in \dsR^{n \times \calC}$, where $\bL_{j, ic} = 1$ if the $i$-th instance is assigned to the $c$ class, otherwise $\bL_{j, ic} = 0$ (as shown in Figure~\ref{fig:framework}).
...
\label{theorem:alm_convergence}
  %Suppose that the sequence $\{ \blambda_{k} \}$ is bounded, any accumulation point
  %\footnote{the limit point of a sequence}~\cite{oh2016partial} $(\bX^*, \bE^*)$ of Algorithm~\ref{alg:alm_mf} is a stationary point.
...
%       & = P_{\U} \G P_{V} + \G P_{V} - \U (\U^{\trsp} \G \V) \V^{\trsp} + P_{U} \G - \U (\U^{\trsp} \G \V) \V^{\trsp} \nonumber \\
...
Ours                                  &   3.47    &  17.94     \\\hline
\end{tabular}
\caption{Test error of different methods on CIFAR-10 and CIFAR-100 with the same training strategies as the original paper~\cite{he2016identity,zagoruyko2016wide,lin2013network}.}
\label{table:state_of_the_art}
\end{table}

As we can observe, our fusion is capable of further improving the performance of single model.
