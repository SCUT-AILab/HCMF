...
(SIFT)~\cite{loweijcv2004distinctive}, Histograms of Gradients (HOG)~\cite{dalalcvpr2005histograms} and trained convolutional neural networks (CNNs) features~\cite{krizhevsky2012imagenet}.
...
%\end{align}
%\noindent
%$L$ is the indicator matrix, represent the label indicator matrix in our vision recoginition case.
%X is a low rank matrix recovered from L. Rank function measures the rank of a matrix. $|| \bX ||_{1,2} = \sum_{j = 1}^{M} \sqrt{\sum_{i=1}^{N} \bX_{ij}} = \sum_{j=1}^{M} || \bX_{.j} ||_2$, where $\bX \in \dsR^{N \times M}$.
...
In this section, we first elaborate the formulation of ORLF.
Then we show the details of the application of ALM to extend matrix factorization to handle $\ell_{1,2}$ loss.
...
\end{align}
Problem~(\ref{eq:subproblem_wrt_X}) contains a smooth loss function \wrt. $\bX$, thus can be efficiently solved by existing fixed-rank matrix factorization optimization algorithms, such as LRGeomCG~\cite{vandereycken2013lowrank}%\footnote{\yanred{Code available at~\url{http://www.unige.ch/math/vandereycken/matrix_completion.html}}}.




Similarly, to update $\bE$, we consider the augmented Lagrangian function \wrt. $\bE$ as below:
\begin{align}\label{eq:subproblem_wrt_E}
...
\begin{small}
  ...
  \bibliography{ensemble_clustering,m3frp}
\end{small}
\end{quote}
